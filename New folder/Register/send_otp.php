...
if (!$isResend) {
    // FIRST REQUEST: validate all fields + CAPTCHA
    ...
    // Phone number validation (basic: allow digits, spaces, +, -, parentheses)
    // You can adjust the regex to your needs (e.g., Philippine mobile numbers)
    ...
    if (!isset($_SESSION['captcha_code']) || $captcha !== strtolower($_SESSION['captcha_code'])) {
        ...
        echo json_encode(['success' => false, 'error' => 'Incorrect CAPTCHA. Please try again.', 'refreshCaptcha' => true]); exit;
    }
    unset($_SESSION['captcha_code']); // CAPTCHA used once
    ...
    // OPTIONAL: Check phone number uniqueness (recommended)
    ...
    // Store pending user including phone number
    $_SESSION['pending_user'] = [
        'firstName' => $firstName,
        'lastName'  => $lastName,
        'email'     => $email,
        'phone'     => $phone,   // NEW: store phone
        'username'  => $username,
  ...
  <tr><td align="center">
    <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
      <tr><td style="background:#111;padding:32px 40px;text-align:center;">
          <p style="margin:0;font-size:26px;font-weight:700;color:#fff;">LookGood</p>
          <p style="margin:6px 0 0;font-size:13px;color:#aaa;">LOOKING GOOD HAS NEVER BEEN THIS CLEAR</p>
        </td>
      </tr>
       <tr><td style="padding:40px 40px 32px;">
          <p style="margin:0 0 8px;font-size:22px;font-weight:600;color:#111;">Verify your email, {$fullName}</p>
          <p style="margin:0 0 28px;font-size:15px;color:#555;">Use the code below to complete your LookGood registration. It expires in <strong>15 minutes</strong>.</p>
       ...
       <tr><td style="background:#f5f5f5;padding:20px 40px;text-align:center;">
          <p style="margin:0;font-size:12px;color:#aaa;">© 2026 LookGood · All rights reserved</p>
        </td>
      </table>
    </table>
    </td>
