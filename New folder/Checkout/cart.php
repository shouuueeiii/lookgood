...
async function loadData() {
    ...
    if (LG_IS_LOGGED_IN) {
        try {
            ...
            if (data.success && Array.isArray(data.items)) {
                cartItems = data.items.map(row => ({
                    ...
                    image:    row.image    ? `../../uploads/products/${row.image}` : '',
                    stock:    row.stock    !== undefined ? Number(row.stock) : 999,
        ...
        // Guest: try DB guest_carts first, fall back to localStorage
        try {
            ...
            if (data.success && Array.isArray(data.items) && data.items.length > 0) {
                ...
                // Sync DB cart back into localStorage as source of truth
    ...
    // Refresh live stock from DB for all items
...
function saveCart() {
    const plain = cartItems.map(i => ({ id: i.id, name: i.name, price: i.price, image: i.image, quantity: i.quantity, brand: i.brand }));
    localStorage.setItem('lookgood_cart', JSON.stringify(plain));
}

// Persist a single item quantity change to DB (logged-in users)
async function persistCartUpdate(productId, quantity) {
    if (!LG_IS_LOGGED_IN) {
        // Guest: update DB guest cart
        try {
            await fetch(GUEST_CART_API, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'update', product_id: productId, quantity })
            });
        } catch(e) { console.warn('Guest cart DB sync failed:', e); }
        return;
    }
    try {
