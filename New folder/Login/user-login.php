        ...
        if (!$user_stmt) {
            ...
            if ($user_result->num_rows === 0) {
                ...
                if (!password_verify($password, $user['password'])) {
                    ...
                    // Save guest_id BEFORE switching session scope (we need it for cart merge)
                    $guestIdBeforeLogin = $_SESSION['guest_id'] ?? null;
                    ...
                    // ── GUEST CART MERGE ──────────────────────────────────────────
                    // Restore guest_id into the new session so mergeGuestCart can find it
                    if ($guestIdBeforeLogin) {
                        $_SESSION['guest_id'] = $guestIdBeforeLogin;
                        debug_log("guest_id preserved for cart merge: " . $guestIdBeforeLogin);
                    }
                    session_write_close();

                    // Merge guest cart into user cart (server-side, inline)
                    if ($guestIdBeforeLogin) {
                        debug_log("Triggering guest cart merge...");
                        ob_start();
                        // Re-open session briefly for mergeGuestCart
                        session_start();
                        require __DIR__ . '/../../userBack_end/mergeGuestCart.php';
                        $mergeOutput = ob_get_clean();
                        $mergeResult = json_decode($mergeOutput, true);
                        debug_log("Cart merge result: " . $mergeOutput);
                        session_write_close();
                    }
                    // ─────────────────────────────────────────────────────────────
