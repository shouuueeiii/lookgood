...
require_once($_SERVER['DOCUMENT_ROOT'] . '/lookgood/PHPMailer/src/PHPMailer.php');
require_once($_SERVER['DOCUMENT_ROOT'] . '/lookgood/PHPMailer/src/SMTP.php');
require_once($_SERVER['DOCUMENT_ROOT'] . '/lookgood/PHPMailer/src/Exception.php');
...
// Determine request type: 'register' (first step) or 'resend'
$action = $input['action'] ?? 'register';   // frontend should send this
...
// ========== CAPTCHA VALIDATION for all requests ==========
...
// ------------------------------------------------------------------
// NEW REGISTRATION (first step)
// ------------------------------------------------------------------
if (!$isResend) {
    // ✅ Clear any existing pending user to avoid old data contamination
    ...
    if (!preg_match('/^[0-9+\-\s()]{8,20}$/', $phone)) {
        echo json_encode(['success' => false, 'error' => 'Please enter a valid phone number (8-20 digits, +, -, spaces allowed).']);
        exit;
    }
    if (!preg_match('/^[a-zA-Z0-9_]{3,20}$/', $username)) {
        echo json_encode(['success' => false, 'error' => 'Username must be 3–20 characters (letters, numbers, underscore).']);
        exit;
    }
    ...
    if ($checkPhone->num_rows > 0) {
        $checkPhone->close();
        echo json_encode(['success' => false, 'error' => 'That phone number is already registered.']);
        exit;
    }
    $checkPhone->close();

    // Store fresh pending user data
...
try {
    ...
    // Rollback session on error only for new registration
    if (!$isResend) {
