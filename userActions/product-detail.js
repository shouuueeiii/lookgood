// ============================================================
        ...
        const PRODUCTS_API_DETAIL = '/lookgood/userBack_end/productsAPI.php';
        ...
        // getSoldCount removed — real count comes from productsAPI (soldCount field)
        ...
        async function loadProduct(productId) {
          ...
          if (categoryLink) categoryLink.innerHTML = '<a href="../home/products-page.php?filter=' + currentProduct.category + '">' + currentProduct.category.charAt(0).toUpperCase() + currentProduct.category.slice(1) + '</a>';
          ...
          // Sold count from real order_items data
          const soldEl = document.getElementById('soldNumber');
          if (soldEl) soldEl.textContent = product.soldCount ?? 0;
          ...
          // Check wishlist status AFTER all DOM updates so wishlistBtn is guaranteed in the DOM
          requestAnimationFrame(() => checkWishlistStatus(product.id));
        }
        ...
        async function loadRelatedProducts(category, currentId) {
          ...
          grid.querySelectorAll('.product-col').forEach(col => {
            col.addEventListener('click', (e) => {
              ...
              window.location.href = `../home/product-detail.php?id=${col.dataset.productId}`;
            });
          });
        ...
        async function loadReviews(productId, category) {
          ...
          // If API failed entirely, show empty state
          if (!data) {
            data = { avg: 0, count: 0, breakdown: {5:0,4:0,3:0,2:0,1:0}, reviews: [] };
          }
        ...
        // ============================================================
        // WISHLIST STATUS — check DB on page load to set heart state
        // ============================================================
        async function checkWishlistStatus(productId) {
          try {
            const res = await fetch('../userBack_end/wishlistAPI.php');
            if (!res.ok) return; // not logged in or error — leave heart default (unfilled)
            const wishlistItems = await res.json();
            if (!Array.isArray(wishlistItems)) return;

            const isWishlisted = wishlistItems.some(item => String(item.id) === String(productId));
