...
// Guest fallback: load address from localStorage only (used when not logged in)
function loadDefaultAddress() {
    // Only used for guests — logged-in users get DB data via loadProfileDefaultAddress()
    if (typeof LG_IS_LOGGED_IN !== 'undefined' && LG_IS_LOGGED_IN) return;
...
// Load profile + default address from DB (logged-in users only)
async function loadProfileDefaultAddress() {
    // Skip entirely for guests — they use localStorage via loadDefaultAddress()
    if (typeof LG_IS_LOGGED_IN === 'undefined' || !LG_IS_LOGGED_IN) return;

    // Force-set helper: always overwrites with DB value (DB is source of truth for logged-in users)
    const set = (id, val) => {
    ...
    try {
        // 1. Load profile info (fullName, email, phone) from profileAPI
        const profileRes = await fetch('/lookgood/userBack_end/profileAPI.php');
        if (profileRes.ok) {
    ...
    try {
        // 2. Load default address from addressAPI
        const res = await fetch('/lookgood/userBack_end/addressAPI.php');
        if (!res.ok) return;
        const addresses = await res.json();
        if (!Array.isArray(addresses) || !addresses.length) return;

        // Find the address marked as default, fallback to first
        const defaultAddr = addresses.find(a => a.is_default === 1) || addresses[0];
        if (!defaultAddr) return;

        set('address1', defaultAddr.address_line1);
        set('address2', defaultAddr.address_line2);
        set('city',     defaultAddr.city);
        set('province', defaultAddr.province);
        set('zip',      defaultAddr.zip_code);
        set('region',   defaultAddr.region);

        const noteEl = document.getElementById('deliveryNote');
        if (noteEl && defaultAddr.delivery_notes) noteEl.value = defaultAddr.delivery_notes;
    } catch(e) {
        // silently fail — not a blocking error
    }
}
...
(async function init() {
    ...
    loadDefaultAddress();
    await loadProfileDefaultAddress();
