...
const STATUS_CONFIG = {
  'paid': {
    key: 'paid', label: 'Paid', badgeClass: 'status-pending', icon: 'fa-check-circle',
    actions: () => '<span class="order-paid-badge"><i class="fas fa-check"></i> Paid</span>'
  },
  'to-ship': {
    key: 'processing', label: 'To Ship', badgeClass: 'status-processing', icon: 'fa-box',
    actions: (order) => `
  ...
  'to-receive': {
    key: 'shipped', label: 'To Receive', badgeClass: 'status-shipped', icon: 'fa-truck',
  ...
  'delivered': {
    key: 'delivered', label: 'Delivered', badgeClass: 'status-delivered', icon: 'fa-check-circle',
  ...
  'completed': {
    key: 'completed', label: 'Completed', badgeClass: 'status-completed', icon: 'fa-history',
  ...
  'cancelled': {
    key: 'cancelled', label: 'Cancelled', badgeClass: 'status-cancelled', icon: 'fa-times-circle',
...
// ── Ratings (stored locally — no separate ratings table required) ─────────────
function loadRatings()                         { try { return JSON.parse(localStorage.getItem('productRatings') || '{}'); } catch { return {}; } }
...
async function loadOrders() {
  ...
  Object.keys(STATUS_CONFIG).forEach(tabKey => {
...
function distributeOrders() {
  for (const [tabKey, config] of Object.entries(STATUS_CONFIG)) {
    const filtered = allOrders.filter(o => o.status === config.key);
    const listEl   = document.getElementById('list-' + tabKey);
  ...
  const itemsHtml = order.items.map(item => {
    ...
    const ratingDisplay  = existingRating ? `
      <div class="rating-display" style="margin-top:0.5rem;font-size:0.8rem;">
        ...
        <span style="color:#666;">"${escapeHtml(existingRating.comment)}"</span>
      </div>` : '';
    const rateButton = (tabKey === 'completed') ? `
      <button class="btn-rate-product-yellow" data-product-id="${item.id}" data-product-name="${escapeHtml(item.name)}"
...
function openRatingModal(productId, productName) {
  currentProductForRating = productId;
  ...
  document.querySelectorAll('.star').forEach(s => { s.classList.remove('active'); s.innerText = '☆'; });
...
function attachRatingButtons() {
  ...
  document.querySelectorAll('.btn-rate-product-yellow').forEach(btn => {
    btn.addEventListener('click', () => openRatingModal(btn.dataset.productId, btn.dataset.productName));
  });
}
