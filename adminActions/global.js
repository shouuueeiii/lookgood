...
function initProfileFeatures() {
    ...
    // ── Push all admin data into every profile UI element ─────
    function applyAdminData(a) {
        if (!a) return;

        // Header trigger: "First L." format
        const parts     = (a.admin_name || '').trim().split(/\s+/);
        const shortName = parts.length >= 2
            ? `${parts[0]} ${parts[parts.length - 1][0]}.`
            : (a.admin_name || '');

        const nameDisplay = document.getElementById('profileNameDisplay');
        if (nameDisplay) nameDisplay.textContent = shortName;
        ...
        // Modal card display labels
        const epCardName  = document.getElementById('epCardName');
        const epCardEmail = document.getElementById('epCardEmail');
        if (epCardName)   epCardName.textContent  = a.admin_name || '';
        if (epCardEmail)  epCardEmail.textContent = a.email      || '';

        // Avatar initials bubble
        const epInitials = document.getElementById('epAvatarInitials');
        if (epInitials)  epInitials.textContent   = makeInitials(a.admin_name);
    ...
    document.getElementById('saveProfile')?.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Sync ALL profile UI elements with fresh DB data
                if (data.admin) applyAdminData(data.admin);
    ...
    // ── Profile image (local preview — no DB column for it) ───
...
// ── Hide sidebar links the current role cannot access ─────────
...
// ── Lightweight toast (falls back to alert if element missing) ─
...
// ============================================================
// NOTIFICATIONS  (header dropdown + full page)
// ============================================================
