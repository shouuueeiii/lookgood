...
function openConfirmModal(order, nextStatus) {
    ...
    confirmModalBtn.onclick = () => {
        ...
        fetch('../adminBack_end/orders.php', {
            ...
            body: JSON.stringify({ id: order.orderId, status: nextStatus.toLowerCase(), source: order.source || 'user' })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
...
document.getElementById('paymentsTable')?.addEventListener('click', (event) => {
    ...
    if (refundBtn) {
        ...
        fetch('../adminBack_end/orders.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: order.orderId, status: 'refunded', source: order.source || 'user' })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
