...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('../adminBack_end/guestAPI.php')
    .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
    })
    .then(data => {
        console.log('Guest data:', data); // tingnan natin kung ano ang nire-return
        const el = document.getElementById('totalVisitors');
        if (el) el.textContent = (data.total_guests ?? 0).toLocaleString();
    })
    .catch(err => {
        console.error('Guest fetch error:', err); // tignan ang exact error
        const el = document.getElementById('totalVisitors');
